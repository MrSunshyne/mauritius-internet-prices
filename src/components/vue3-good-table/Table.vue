<template>
    <div :class="wrapStyleClasses">
        ...
        <div class="vgt-inner-wrap" :class="{ 'is-loading': isLoading }">
            ...
            >
                <vgt-pagination
                    ...
                    :perPageDropdownEnabled="
                        paginationOptions.perPageDropdownEnabled
                    "
            ...
            <vgt-global-search
                @on-keyup:modelValue="searchTableOnKeyUp"
                @on-enter:modelValue="searchTableOnEnter"
                v-model="globalSearchTerm"
                :search-enabled="searchEnabled && externalSearchQuery == null"
                :global-search-placeholder="searchPlaceholder"
            >
                <template
                    v-slot:internal-table-actions
                    v-if="hasTableActionsSlot"
                >
                    ...
                    >
                        <template v-slot:table-column="props">
                            <slot name="table-column" :column="props.column">
                                <span data-v-step="overview#2">{{ props.column.label }}</span>
                            </slot>
                        </template>
...
export default {
    ...
    computed: {
        ...
        selectedRows() {
            ...
            return selectedRows.sort(
                (r1, r2) => r1.originalIndex - r2.originalIndex
            );
        },
                    ...
                    cRows.children.sort((xRow, yRow) => {
                        ...
                        for (let i = 0; i < this.sorts.length; i += 1) {
                            ...
                            if (srt.type === SORT_TYPES.None) {
                                ...
                                sortValue =
                                    sortValue ||
                                    xRow.originalIndex - yRow.originalIndex;
                            } else {
                                const column = this.getColumnForField(
                                    srt.field
                                );
                                ...
                                if (sortFn && typeof sortFn === 'function') {
                                    sortValue =
                                        ...
                                        sortFn(
                                            ...
                                            (srt.type === SORT_TYPES.Descending
                                                ? -1
                                                : 1);
                                } else {
        ...
        paginated() {
            ...
            if (this.paginate) {
                ...
                if (
                    pageStart >= paginatedRows.length ||
    ...
    methods: {
        ...
        toggleSelectAll() {
            ...
            const rows = this.selectAllByPage
                ? this.paginated
                : this.filteredRows;
        ...
        getClasses(index, element, row) {
            const { typeDef, [`${element}Class`]: custom } = this.typedColumns[
                index
            ];
            let { isRight } = typeDef;
                ...
                for (let i = 0; i < this.typedColumns.length; i++) {
                    ...
                    if (this.columnFilters[fieldKey(col.field)]) {
                        computedRows.forEach((headerRow) => {
                            const newChildren = headerRow.children.filter(
                                (row) => {
                                    // If column has a custom filter, use that.
                                    if (
                                        col.filterOptions &&
                                        typeof col.filterOptions.filterFn ===
                                            'function'
                                    ) {
        ...
        handleGrouped(originalRows) {
            originalRows.forEach((headerRow, i) => {
                ...
                if (
                    ...
                    this.expandedRowKeys.has(
                        headerRow[this.groupOptions.rowKey]
                    )
                ) {
                    headerRow.vgtIsExpanded = true;
                }
